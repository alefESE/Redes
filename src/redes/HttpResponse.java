...
public class HttpResponse {
	...
	try {
	    String line = fromServer.readLine();//preenchido deprecated
	    while (line.length() != 0) {
		if (!gotStatusLine) {
		    statusLine = line;
		    gotStatusLine = true;
		 ...
		 * here. */
		if (line.startsWith("Content-Length:") ||//preenchido
	...
	try {
		...
		int res = fromServer.read(buf);//preenchido
		if (res == -1) {
		    break;
		}
		...
		for (int i = 0; 
		     ...
		     i++) {
		    body[i + bytesRead] = buf[i];//preenchido
		}
		bytesRead += res;
	    }
 	} catch (IOException e) {
